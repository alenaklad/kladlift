...
export function CoachView({ user, workouts, onBack }: CoachViewProps) {
      ...
      <div className="flex flex-col h-screen bg-slate-50 dark:bg-slate-950">
        <div className="p-6 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Команда KladLift</h1>
          </div>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Твой персональный штаб экспертов.</p>
        </div>
          ...
          {(Object.entries(COACH_PERSONAS) as [CoachPersonaId, typeof COACH_PERSONAS[CoachPersonaId]][]).map(([id, coach]) => {
            ...
            return (
              <button 
                ...
                className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800 text-left flex items-center gap-5 hover:border-purple-300 dark:hover:border-purple-700 hover:shadow-md transition-all active:scale-[0.98]"
              ...
              >
                ...
                <div>
                  <h3 className="text-xl font-bold text-slate-900 dark:text-white">{coach.name}</h3>
                  <p className="text-slate-500 dark:text-slate-400 text-sm mt-1 leading-snug">{coach.desc}</p>
                </div>
              </button>
            );
          })}
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 dark:bg-slate-950">
      <div className="flex items-center p-4 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 sticky top-0 z-20">
        <button 
          onClick={() => setActiveCoachId(null)} 
          className="p-2 -ml-2 mr-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 transition-colors"
        ...
        <div>
          <h2 className="font-bold text-slate-900 dark:text-white">{activeCoach.name}</h2>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div 
              className={`max-w-[80%] p-4 rounded-3xl ${
                msg.role === 'user' 
                  ...
                  : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-bl-lg shadow-sm border border-slate-200 dark:border-slate-700'
              }`}
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="bg-white dark:bg-slate-800 p-4 rounded-3xl rounded-bl-lg shadow-sm border border-slate-200 dark:border-slate-700">
      ...
      <div className="p-4 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="p-3 bg-slate-100 dark:bg-slate-800 rounded-full text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
          ...
          <input
            ...
            className="flex-1 bg-slate-100 dark:bg-slate-800 rounded-full px-5 py-3 outline-none text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:ring-2 focus:ring-purple-500/20 focus:bg-white dark:focus:bg-slate-700 focus:border focus:border-purple-500 transition-all"
